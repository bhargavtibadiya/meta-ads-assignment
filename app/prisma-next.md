...
Your data contract is the heart of your application. It lives at [`prisma/schema.prisma`](prisma/schema.prisma) (Prisma 8 still calls this a contract; we keep the familiar `schema.prisma` filename) and describes your models:
...
| [`src/config/prisma.ts`](src/config/prisma.ts) | Database client — `import { db } from '../config/prisma.js'` |
| `prisma/contract.json` | Compiled contract (generated, required by Prisma 8) |
| `prisma/contract.d.ts` | Contract types (generated, required by Prisma 8) |
| `prisma/migrations/` | Prisma 8 migration packages (`migration.ts` + `ops.json`, not SQL folders) |
